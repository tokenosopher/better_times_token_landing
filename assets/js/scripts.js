...
function menu_setup() {
	...
	function setupLinks(scroller) {
		...
		//custom code addition: added an array of buttons that can navigate the page
		let linkMainElements = gsap.utils.toArray('.right-content button.scroll-to');

		//separated the code for easy reading:
		let linkMenuElements = gsap.utils.toArray('.m-menu .main-items a.scroll-to')

		let linkElements = linkMainElements.concat(linkMenuElements),
...
function image_setup() {
	...
	//made custom modification: if the class is mobile, then do this:
	if(!$("body").hasClass("mobile")) {
		try { img_three({
			...
			console.log(e, "Blog post mode")
		}
		//reduce the opacity of the static image to 0
		$(".left-content .picture-box .picture").css("opacity", 0)
	}
...
function mobile_layout() {
	...
	ScrollTrigger.create({
		...
		onLeave: function() {
			...
			if(isLandingPage) {
				...
				//custom made change - made the opacity 0 instead of 0.25
				gsap.to(".mobile .left-content .inner-content",{duration:0.86, opacity:0, x:"-75%", rotateY: "50deg", ease:"power3.inOut",zIndex:-2,overwrite: true})
	...
	function resize() {
		...
		if($(window).width() < config_mobile_max_width_for_layout) { 
			...
			// $(".left-content .picture-box .picture").css("opacity", 1);



			if(is_top && isLandingPage) {
				gsap.to(".mobile .left-content",{duration:0.1, zIndex:2,overwrite: true})
				gsap.to(".mobile .left-content .inner-content",{duration:0.86, opacity:1,  x:"0%", rotateY: "0", ease:"power3.inOut",overwrite: true})
				// gsap.to(".mobile .left-content .inner-content .name",{duration:0.4, opacity:1, ease:"power2.inOut",overwrite: true})
...
//function to check if it is the landing page:
function isLandingPageFunc(){
	// var pagePathName= window.location.pathname;
	// let pageName = pagePathName.substring(pagePathName.lastIndexOf("/") + 1);
	// return ((pageName.includes("index") || pageName == "") ? true : false)
	isLandingPage = true;
}
...
$("#contact-form").submit(async function (e) {
		...
		let hp = $("#contact-form-country").val();
		let a_name;

		$('#subscribeBtnText').text("Please wait..")
		...
		if(hp!=="") {
			...
			a_name = await $.post("https://subscribe.sacredcoinprotocol.autocode.gg/", {name:name,
				email: email})
			if (a_name.status == 'subscribed') {
				$('#subscriptionSucceeded').modal({
					fadeDuration: 400
				})

				$('#contact-form-name').attr('disabled', true);
				$("#contact-form-email").attr('disabled', true);

				$('#subscribeBtnText').text("Subscribed!")
				$('#subscribeBtnIcon').hide();
			}
			else {
				$('#subscriptionFailedMsg').text(a_name.title)
				$('#subscriptionFailed').modal({
					fadeDuration: 400
				})
				$('#subscribeBtnText').text("Subscribe")
